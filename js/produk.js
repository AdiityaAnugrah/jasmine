...
async function getAllBarang() {
  ...
  if (kategori) {
    const response = await fetch(
      "http://192.168.1.5:8082/kategori/" + kategori
    );
    barangs = await response.json();
  } else if (subkategori) {
    const response = await fetch(
      "http://192.168.1.5:8082/subkategori/" + subkategori
    );
    barangs = await response.json();
  } else {
    const response = await fetch("http://192.168.1.5:8082/getallbarang");
    barangs = await response.json();
  }
...
elmformsearch.addEventListener("submit", (e) => {
  ...
  async function cariProduk() {
    ...
    const response = await fetch("http://192.168.1.5:8082/cari", {
      method: "POST",
      headers: {
