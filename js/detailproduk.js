...
async function getBarang() {
  ...
  const response = await fetch("http://192.168.1.49:8082/barang/" + id);
  ...
  elmBeli.href = "http://192.168.1.49:8082/product/" + barang.data[0].id;
...
async function getGambarBarang() {
  ...
  const URLparams = new URLSearchParams(queryString);
  const id = URLparams.get("id");
  const response = await fetch("http://192.168.1.49:8082/gambar/" + id);
  const barang = await response.json();
  console.log(barang);
